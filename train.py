...
def get_negative_mask(batch_size):
    negative_mask = torch.ones((batch_size, batch_size), dtype=bool)#torch.ones((batch_size, 2 * batch_size), dtype=bool)
...
def train_ucgl(dataset):
    ...
    for epoch in range(nb_epochs):
        ...
        for n in range(np.max(y_pred) + 1):
            pos_samp[np.where(y_sam.cpu() == n)] = pos.cpu().index_select(1, class_sam_pos[n])[np.where(y_sam.cpu() == n)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain_path', default='youpath' , type=str, help='the pretrained MVGRL path')#pretrained on MVGRL(https://github.com/kavehhassani/mvgrl)
